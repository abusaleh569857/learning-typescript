...
type AverageRatingMap = { [categoryName: string]: number };

const getCategoryAverageRatings = (products: Products[]): AverageRatingMap => {
  ...
  const categoryData = products.reduce((acc, product) => {
    ...
    return acc;
  }, {} as { [key: string]: { totalSum: number; totalReviews: number } });

  
  const result: AverageRatingMap = {};
